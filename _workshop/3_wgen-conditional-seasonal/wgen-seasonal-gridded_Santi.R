...
list.of.packages <- c("ncdf4","MASS","fields","geoR","data.table","plyr","lubridate","sirad","dplyr")  
...
setwd("~/Desarrollo/RProjects/Wth Simulation")
## load the setup data
load("1_setup/wgen-setup.RData")

dir.grid <- '/media/Raid/Repo/Santiago/CNH3/data/grillas_modelo'

grilla.5000.GK <- rgdal::readOGR(dir.grid, layer="Salado_A_grilla_5000_GK", verbose = TRUE)
grilla.5000.ll <- rgdal::readOGR(dir.grid, layer="Salado_A_grilla_5000_latlon", verbose = TRUE)
cuenca_shape <- rgdal::readOGR('/media/Raid/Repo/Santiago/CNH3/data/shapes_cuenca', layer="Salado_A_GK", verbose = TRUE)

predlocs <- cbind(coordinates(grilla.5000.GK), coordinates(grilla.5000.ll))
colnames(predlocs) <- c('x_coord', 'y_coord', 'lon', 'lat')
...
## to use as predictor
# d_montot <- data.frame(year=yr, month=mo, day=da, prcp=PP)
...
## 
sim_start_date <- as.Date('1997-05-01')
sim_end_date <- as.Date('2012-04-30')
sim.dates <- seq(from = sim_start_date, to = sim_end_date, by = "days")

yr.sim <- year(sim.dates)
mo.sim <- month(sim.dates)
da.sim <- day(sim.dates)
ct.sim <- ct[yday(sim_start_date):(length(yr.sim)+yday(sim_start_date)-1)]  # yday = day of year
st.sim <- st[yday(sim_start_date):(length(yr.sim)+yday(sim_start_date)-1)]
## if you wish to neglect temperature trends, and be centered on the mean of 1961-2013 
## temperatures, set Rt.sim = 0 for all days
Rt.sim <- seq(from=-1, to=1,length.out = length(yr.sim))
...
Times <- julian(sim.dates, origin = sim.start.date)
...
for(i in 1:NT){
    for(k in 1:np){
        ...
        SC[,k,i] <- exp(apply(GAMMA[[k]]$coef*rbind(1,ct.sim,st.sim),FUN=sum,MAR=2,na.rm=T))/SH[k]
    }
}
...
for(i in 1:NT){
    ...
    for(d in 1:nt.sim){
        ...
        mu.occ <- apply(rbind(1,SIMocc.old,ct.sim[d],st.sim[d])*coefocc.sim,2,sum,na.rm=T)
        ...
        mu.min <- apply(rbind(1,SIMmin.old,SIMmax.old,ct.sim[d],st.sim[d],SIMocc.sim[d,,i],Rt.sim[d])*coefmin.sim,2,sum,na.rm=T)
        ...
        mu.max <- apply(rbind(1,SIMmin.old,SIMmax.old,ct.sim[d],st.sim[d],SIMocc.sim[d,,i],Rt.sim[d])*coefmax.sim,2,sum,na.rm=T)
...
save.image(file='post-calc.RData')
...
rm(tavg, PROBIT, predrad, day.of.year); gc();

# doy_matrix <- matrix(rep(day.of.year,nrow(predloc.GK)),nrow=nt.sim,ncol=nrow(predloc.GK),byrow=F)
# extraT_matrix <- extrat(i=doy_matrix, lat=predrad)$ExtraTerrestrialSolarRadiationDaily
# 
# extraT <- array(
#     suppressWarnings(
#         
...
#                           doy_matrix,
#                        nrow=nt.sim,
#                        ncol=nrow(predloc.GK),
#                        byrow=F),
#                lat=predrad)$ExtraTerrestrialSolarRadiationDaily
#     ),
#     dim=dim(dtr)
# )

SIMsrad.sim <- q0 * bc.coef[['A']] * (1 - exp(-bc.coef[['B']] * (dtr^bc.coef[['C']])))
rm(q0, dtr); gc();

save.image(file='post-calc-srad.RData')
...
# dimTime <- ncdim_def(name="time", units=, vals=, unlim = FALSE, create_dimvar=TRUE, calendar="standard", longname="Time in days since 1961-01-01")
...
# dec_lon <- ncvar_def(name="dec_lon", units="degrees_east", missval=mv, dim=list(dimX, dimY), longname="Longitude in decimal degrees", prec="float", verbose=TRUE)
# dec_lat <- ncvar_def(name="dec_lat", units="degrees_north", missval=mv, dim=list(dimX, dimY), longname="Latitude in decimal degrees", prec="float", verbose=TRUE)
...
dimX <- ncdf4::ncdim_def(name="projection_x_coordinate",
                         ...
                         unlim = FALSE, 
                         create_dimvar = TRUE,
                         longname="x-coordinate (longitude) in Gauss-Kruger projection")
...
dimY <- ncdf4::ncdim_def(name="projection_y_coordinate",
                         ...
                         unlim = FALSE, 
                         create_dimvar = TRUE,
                         longname = "y-coordinate (latitude) in Gauss-Kruger projection")

dimTime <- ncdf4::ncdim_def(name = "time",
                            units = paste0("Days since ", as.character(attr(Times, 'origin'))),
                            vals = Times,
                            unlim = FALSE, 
                            create_dimvar = TRUE,
                            calendar = "standard",
                            longname = "Time of simulated values (days)")

grid.coords  <- coordinates(grilla.5000.ll)
colnames(grid.coords) <- c('lon', 'lat')
dec_lon_vals <- grid.coords[, 'lon']
dec_lat_vals <- grid.coords[, 'lat']

dec_lon <- ncdf4::ncvar_def(name = "dec_lon",
                            units = "degrees_east",
                            missval = mv,
                            dim =  list(dimX, dimY),
...
dec_lat <- ncdf4::ncvar_def(name = "dec_lat",
                            units = "degrees_north",
                            missval = mv,
                            dim =  list(dimX, dimY),
...
dir.dem <- '/media/Raid/Repo/Santiago/CNH3/data/DEM_cuenca/'
dem.csv <- paste0(dir.dem, 'Salado_A_DEM_5000m_GK_v1.0.txt')
if (file.exists(dem.csv)) {
    dem.xyz <- read.csv(file = dem.csv, sep = '\t', header = T) %>%
        dplyr::arrange(latitude, longitude)
} else {
    Log.error(paste0("El archivo DEM no existe: ", dem.csv))
}

# --- Save elevation values to store in netCDF file

elev_vals <- dem.xyz[, 'elevation']

elevation <- ncdf4::ncvar_def(name = "elevation",
                              units = "meters",
                              missval = mv,
                              dim =  list(dimX, dimY),
                              longname = "Terrain elevation (from DEM version 1.0 by INA)",
                              prec = "float",
                              verbose = TRUE)

cell.id <- ncdf4::ncvar_def(name = "cell_id",
                            units = "(none)",
                            missval = mv,
                            dim =  list(dimX, dimY),
                            longname = "Cell Id (starting from SW, then moving to E and N)",
                            prec = "float",
                            verbose = TRUE)

tmax <- ncvar_def(name="tmax", units="degress_Celsius", dim=list(dimRNum, dimX, dimY, dimTime), missval=mv, longname="Daily maximum near-surface air temperature", prec="float", verbose=TRUE)
tmin <- ncvar_def(name="tmin", units="degress_Celsius", dim=list(dimRNum, dimX, dimY, dimTime), missval=mv, longname="Daily minimum near-surface air temperature", prec="float", verbose=TRUE)
prcp <- ncvar_def(name="prcp", units="mm day-1", dim=list(dimRNum, dimX, dimY, dimTime), missval=mv, longname="Daily total rainfall", prec="float", verbose=TRUE)
srad <- ncvar_def(name="srad", units="Mjoules m-2 day-1", dim=list(dimRNum, dimX, dimY, dimTime), missval=mv, longname="Daily solar radiation", prec="float", verbose=TRUE)
et0  <- ncvar_def(name="et0",  units="mm day-1", dim=list(dimRNum, dimX, dimY, dimTime), missval=mv, longname="Reference evapotranspiration", prec="float", verbose=TRUE)

# elevation <- ncvar_def(name="elevation", units="meters from sea level", missval=mv, dim=list(dimX, dimY), longname="Elevation in meters from sea level", prec="float", verbose=TRUE)
...
dd <- nc_create(filename="wgen-conditional-seasonal-gridded-short.nc",
                # vars = list(tmax, tmin, prcp, srad, et0, dec_lon, dec_lat, elevation, STs, SMXs, SMNs),
                vars = list(tmax, tmin, prcp, srad, et0, dec_lon, dec_lat, elevation, cell.id),
                force_v4 = TRUE, verbose = TRUE)
# 
# 
# nw_limit <- c(x=min(grid_points$x), y=max(grid_points$y))
# ne_limit <- c(x=max(grid_points$x), y=max(grid_points$y))
# se_limit <- c(x=max(grid_points$x), y=min(grid_points$y))
# sw_limit <- c(x=min(grid_points$x), y=min(grid_points$y))
# 
# limits_df <- data.frame(rbind(nw_limit, ne_limit, se_limit, sw_limit, nw_limit))
# colnames(limits_df) <- c('x', 'y')
# 

# coordinates(mtrx_points) <- grid_points
# mtrx <- matrix(SIMmin.sim[d,,i],nrow=n.y_coords,ncol=n.x_coords,byrow=T)
grid_points_df <- data.frame(predloc.GK*1000, order=seq(from=1, to=nrow(predloc.GK)))

# full_grid_points <- expand.grid(x=x_coords, y=y_coords) %>%
#                         mutate(row_num=row_number()) %>%
#                         full_join(grid_points_df, by=c('x'='x_coord', 'y'='y_coord'))
...
data_access_order <- full_grid_points[which(!is.na(full_grid_points$order)), 'order']
data_set_order <- full_grid_points[which(!is.na(full_grid_points$order)), 'row_num']

# test_grid_df <- rep(NA, times=nrow(full_grid_points))
# test_grid_df[data_set_order] <- SIMmin.sim[1,,1][data_access_order]

single_dimension_values <- rep(NA, times=nrow(full_grid_points))
single_dimension_values[which(is.na(full_grid_points$order))] <- mv


# require(ggplot2)
# rain_day_one <- data.frame(grid_points, prcp=SIMamt.sim[1,,1])
# ggplot(data=rain_day_one, aes(x=x_coord, y=y_coord, fill=prcp)) + geom_raster(interpolate=F)
# 
# dim_values <- single_dimension_values
# dim_values[data_set_order] <- SIMamt.sim[1,,1][data_access_order]
# store_matrix <- matrix(dim_values,nrow=n.y_coords,ncol=n.x_coords,byrow=T)
# 
# # colnames(store_matrix) <- as.integer(x_coords)
# # rownames(store_matrix) <- as.integer(y_coords)
# 
# matrix_as_df <- data.frame(store_matrix)
# plot(matrix_as_df)
...
for(i in 1:NT){
    ...
    for(d in d:nt.sim){
        # dim_values <- single_dimension_values
        # dim_values[data_set_order] <- SIMmax.sim[d,,i][data_access_order]
        
        ncvar_put(dd, varid = "tmax", 
                  # vals = matrix(dim_values, nrow=n.y_coords,ncol=n.x_coords),
                  vals = SIMmax.sim[d,,i],
                  start = c(i, 1, 1, d), count = c(1, n.x_coords, n.y_coords, 1),
                  verbose = F)
        
        
        # dim_values <- single_dimension_values
        # dim_values[data_set_order] <- SIMmin.sim[d,,i][data_access_order]
        ...
        # dim_values <- single_dimension_values
        # dim_values[data_set_order] <- SIMamt.sim[d,,i][data_access_order]
        ...
        # dim_values <- single_dimension_values
        # dim_values[data_set_order] <- SIMet0.sim[d,,i][data_access_order]
        ...
        # dim_values <- single_dimension_values
        # dim_values[data_set_order] <- SIMsrad.sim[d,,i][data_access_order]
        
        ncvar_put(dd, varid = "srad",
                  # vals = matrix(dim_values,nrow=n.y_coords,ncol=n.x_coords),
                  vals = SIMsrad.sim[d,,i],
                  start = c(i, 1, 1, d), count = c(1, n.x_coords, n.y_coords, 1),
                  verbose = F)
        cat(paste0(d,'/',nt.sim,'\r\r'))
    }
    setTxtProgressBar(pb, i)
}
...
ncvar_put(dd, varid = "dec_lon",
          vals = dec_lon_vals,
          start = c(1, 1), count = c(n.x_coords, n.y_coords),
          verbose = TRUE)
...
ncvar_put(dd, varid = "elevation",
          vals = dem.xyz$elevation,
          start = c(1, 1), count = c(n.x_coords, n.y_coords),
          verbose = TRUE)

ncvar_put(dd, varid = "cell_id",
          vals = as.numeric(grilla.5000.GK$ID),
          start = c(1, 1),
          count = c(n.x_coords, n.y_coords),
          verbose = TRUE)
...
ncatt_put(dd, varid = 0,
          attname = "start_date",
          attval = as.character(sim.start.date),
          verbose = TRUE)
