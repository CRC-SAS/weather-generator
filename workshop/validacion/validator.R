...
package.list <- c("Cairo", "dplyr", "dplyr", "ggplot2", "lubridate", "reshape2", 
                  "rmarkdown", "tidyr", "yaml", "xts", "magrittr")
...
# Cargar estaciones y datos observados
input.stations <- paste0(config$dir$base, "/input/", config$input.file, ".RData")
load(input.stations)
rm(input.stations)
...
#netcdf.extract.points.as.tibble(netcdf_filename = netcdf.file, 
#                                points_to_extract = stations)


#datos <- netcdf.as.tibble(netcdf_filename = netcdf.file) %>%
#  dplyr::mutate(., station_id = stations$station_id)
# ------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------#
# --- PASO 7. Efectuar validacion y almacenar resultados ----
# -----------------------------------------------------------------------------#
# Proceso de generacion de validaciones
# 
# Stations debe tener station_id. Filtrar estacion. 
stations <- stations %>%
  dplyr::mutate(lat_dec = sf::st_coordinates(geometry)[,'Y'])
# Estaciones ajustadas. Si las estaciones con la que se realizó la calibración 
# son las mismas 
